...
int main(int argc,char *argv[])	{
	int t0 = clock();
	setbuf(stdout,NULL);
	printf("\nArchon3r3\t(C)kvark, 2006");
	if(argc < 4)	{
		printf("\nCall format:");
		printf("\narchon3r3 [e|d] <source> <dest>");
	...
	if(argv[1][0] == 'e')	{
		...
		#if VERBLEV >= 2
		printf("\nRoutine calls: NumSt(%d)",numst);
		printf("\n\tDigger(%d), LazyRay(%d)",call_digger,call_lazy_ray);
...
*/
void lcp_qsort(int P[], int a, int b, uchar *bof)	{
	do	{ int x=a,y=a,z=b;
		short wk = lcp[(a+b)>>1];
		do	{ short qk = lcp[y];
			int s = P[y];
		...
		if(a+1 < x) lcp_qsort(P,a,x,bof);
		if(x+1 < y)	{ int cur = lcp[x];
			if(cur <= 0) cur += LENMAX;
			else cur = LENMAX - cur;
			deep_ray(P+x,P+y,bof-(cur<<2));
		}a=z;
	}while(b-a > 1);
}
...
*/
void lazy_ray(int A[], const int num, uchar bof[])	{
	int i,s;
